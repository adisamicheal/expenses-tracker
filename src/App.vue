<template>
  <div id="app">
    <Navbar :expense="expenses"  @update-home="updateHome" :total-expense="addedExp" />
    <div style="overflow: auto;">
      <div class="grid-container">
        <div class="card card" style="width: 18rem;" v-for="(expense, key) in expenses" :key="key">
          <div class="card-body">
            <p><b>Item:</b> {{ expense.tell }}</p>
            <p><b>Amount Spent:</b> {{ expense.amount }}</p>
            <p><b>Any Backing Reason:</b> {{ expense.details }}</p>
          </div>
        </div>
      </div>
    </div>
  ...
  Navbar
    ...
    methods: {
      updateHome(item) {
        ...
        this.addedExp = parseFloat(this.addedExp) + parseFloat(item.amount);
